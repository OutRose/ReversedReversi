...
- ✅ **マジックナンバー定数化** (β-D-4 完了、部分): 画面・FPS・盤面サイズ・主要レイアウト座標を GameMain.h に集約。残りパネル詳細レイアウト座標は β-D-5 で対応
- **盤面ロジックの単一化** (β-D-5 候補、未着手): Game1Scene と Game2Scene (旧 Game4) の 80% 以上が `2`/`A` 接尾辞違いのコードクローン。`ReversiBoard` 構造体 (`int board[BOARD_SIZE][BOARD_SIZE]`, `std::string msg`, `int msg_wait`) と `putPiece(state, x, y, ...)` のような state 引数渡しに統合できる (490 + 380 行 ≒ 870 行から数百行削減可能)
- **シーン間共有構造体** (β-D-5 関連、未着手): TwistTimeStopper の `TIMER_STATE` に相当する `ReversiSession` (`SqBoard`, `nameTmp`, `CurrentRound` 等) を `GameSceneMain.h` で定義してシーン間で共有
...
- Game2 (まきもどり、旧 Game4) の補助関数群は `*2` / `*02` サフィックスで Game1 と分離 (`putPiece2`, `think01`, `think02`, `setMsg2`, `checkResult2`) — **完全コードクローンの臭み** (関数名はシーン番号と整合していないが、α-7 ではファイル名/公開関数名のみ更新し補助関数名は据置)。β-D-5 で盤面ロジック統合時に解消予定
...
- グローバル: 名詞 (`SqBoard`, `SqBoardA`, `Input`, `EdgeInput`, `nameTmp`, `CurrentRound`, `excuted`, `FrameStartTime`)
  - `SqBoard` (Game1) と `SqBoardA` (Game2=まきもどり) も同様の `2`/`A` 接尾辞重複
...
4. **β-D: 構造系**
   ...
   - ✅ **β-D-4: マジックナンバー定数化** (完了、部分) — 画面 (`SCREEN_WIDTH/HEIGHT/BPP=16`)、FPS、盤面 (`BOARD_SIZE 12`, `BOARD_SIZE_MAX 11`, `CELL_PX 48`, `PIECE_SIZE_PX 47`, `BOARD_CENTER_LOW/HIGH 5/6`)、`MSG_WAIT_FRAMES 60`、メッセージ/パネルレイアウト座標を [GameMain.h](Project2/GameMain.h) に集約。Game1Scene/Game2Scene 全域で参照置換 (グリッド描画の細部レイアウトは β-D-5 で対応)
   - **β-D-5: 盤面ロジック単一化** (次セッション、未着手) — `Game1Scene` と `Game2Scene` の `putPiece` / `putPiece2` / `think*` / `setMsg*` を `ReversiBoard` 構造体経由の関数群に統合 (約 800 行から 200〜300 行削減見込)
