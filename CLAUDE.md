...
- `renderGame1Scene` / `renderGame2Scene` に描画コードを分離 (描画 6 ブロックの共通関数化は次セッション課題)
...
3. **γ-3: Game3 (あまちゃん次元) の方針確定と実装** (未着手) — [10. 未完成機能の方針](#10-未完成機能の方針) の方針候補 1〜3 から選択
...
4. **描画 6 ブロックの共通関数化** (γ-1 で見送り、次セッション候補) — `rbDrawBoard/Grid/Pieces/Msg/Count/TurnIndicator` を [GameSceneMain.cpp](Project2/GameSceneMain.cpp) に追加し、Game1Scene/Game2Scene の `renderXxxScene` を呼び出しのみに圧縮 (盤面色は引数化)
