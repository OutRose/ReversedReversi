...
## 2. ファイルエンコーディング方針: UTF-8 BOM 統一を目標

**現状: 全 17 ファイル (Project2/ 配下の .cpp / .h) が Shift-JIS (CP932) + CRLF。UTF-8 BOM 化は未実施。** TwistTimeStopper は 2026-06-22 に統一済みで、本プロジェクトも同方針へ移行する。

### 現状

| 種別 | ファイル数 |
|---|---|
| Shift-JIS (CP932) | 17 (全数) |
| UTF-8 BOM | 0 |
| UTF-8 BOM なし | 0 |

改行コードは全ファイル CRLF で混在なし。`.editorconfig` は **無し**。
...
UTF-8 BOM 統一前は MSVC の自動判定が CP932 なので、新規ファイルを Write ツールで作成すると **そのまま CP932 として解釈されてビルドは通る** が、BOM 混在のままだと将来の統一作業で `[char]0xFFFD` 検出に引っかかるので注意。
...
**Debug 構成: `C:\DxLib`** / **Release 構成: `C:\DxlibFile`** とハードコードされており、**Release 構成は `C:\DxlibFile` 不在のため現状ビルド不可** (C1083: `DxLib.h` が見つからない、8 件)。

Debug 構成は `C:\DxLib` 実在のため警告ゼロ・エラーゼロでビルド成功。`d:\Repositories\ReversedReversi\Debug\Project2.exe` がリポジトリにコミット済み。

Release 構成の修正案 (α 候補):
- `C:\DxLib` に統一する (`.vcxproj` の Release 側 `AdditionalIncludeDirectories` / `AdditionalLibraryDirectories` を書き換え)
- もしくは `C:\DxlibFile` に DxLib_VC 一式を配置
- 中長期的には `DXLIB_DIR` 環境変数か `.props` シートに切り出して移植性を確保
...
|---|---|---|---|---|
| Debug | Win32 | 成功 | 0 | 0 |
| Release | Win32 | 失敗 | 0 | 8 (C1083: `DxLib.h` 解決不能) |
...
候補:
1. **エンコーディング統一**: 17 ファイル CP932 → UTF-8 BOM 化、`.editorconfig` 新設 ([2. ファイルエンコーディング方針](#2-ファイルエンコーディング方針-utf-8-bom-統一を目標))
2. **Release 構成のビルド復活**: DxLib パスを `C:\DxlibFile` → `C:\DxLib` に統一 ([3. ビルド環境](#3-ビルド環境))
...
完了済みの方針判断 (再議論不要)。**初版時点では未実施。今後、フェーズ完了ごとに追記する。**
