...
[Project2/GameSceneMain.cpp](Project2/GameSceneMain.cpp) では `switch (sceneNo)` を **`CollideCallback` / `initCurrentScene` / `moveCurrentScene` / `renderCurrentScene` / `releaseCurrentScene` の 5 関数で同じ case 列をコピペ**して 5 箇所のディスパッチを実現している (それぞれが MENU/GAME1〜GAME4 の 6 ケース)。シーン追加コストは「enum 1 行 + switch 5 箇所 + 関数プロトタイプ追加」。
...
- **インクルードガード `__XXX_H_` → `XXX_H_` 統一**: α-6 で完了済 ([5. 命名規約](#5-命名規約) 参照)
- **盤面ロジックの単一化**: Game1Scene と Game2Scene (旧 Game4) の 80% 以上が `2`/`A` 接尾辞違いのコードクローン。`ReversiBoard` 構造体 (`int board[12][12]`, `std::string msg`, `int msg_wait`) と `putPiece(state, x, y, ...)` のような state 引数渡しに統合できる (490 + 380 行 ≒ 870 行から数百行削減可能)
...
- **色**: `GetColor(R, G, B)` の戻り値 `unsigned int` は本来キャッシュすべき。本プロジェクトでは `ColorWhite`, `ColorRed`, `ColorSky` 系のグローバルが Game1/Game2 (=まきもどり) で別々に定義されている (`ColorWhite2` 等) — TwistTimeStopper β-D-2a 相当の集約候補
